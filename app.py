...
def extraer_datos_factura(pdf_path):
    ...
    if es_el_corte_ingles:
        ...
        # Extracción de días específica: busca "Alquiler de contador" y luego el número de días
        patron_dias_nat = r'Alquiler\s+de\s+contador\s*(\d+)\s*días'
        match_dias = re.search(patron_dias_nat, texto_completo, re.IGNORECASE)
        dias = int(match_dias.group(1)) if match_dias else 0

        # Potencia (Suele aparecer como término de potencia o similar)
        match_pot = re.search(r'Potencia\s*([\d,.]+)\s*kW', texto_completo, re.IGNORECASE)
        potencia = float(match_pot.group(1).replace(',', '.')) if match_pot = else 0.0

        # Consumos (Búsqueda genérica por tramos para Naturgy)
        for tramo in ['punta', 'llano', 'valle']:
            m = re.search(rf'{tramo}.*?([\d,.]+)\s*kWh', texto_completo, re.IGNORECASE)
            if m: consumos[tramo] = float(m.group(1).replace(',', '.'))

        # Fecha y Total
        match_fecha = re.search(r'Fecha\s+de\s+factura:\s*([\d/]+)', texto_completo, re.IGNORECASE)
        fecha = match_fecha.group(1) if match_fecha else "No encontrada"
        match_total = re.search(r'Importe\s+total.*?([\d,.]+)\s*€', texto_completo, re.IGNORECASE | re.DOTALL)
        total_real = float(match_total.group(1).replace(',', '.')) if match_total else 0.0
        ...
        consumos = {'punta': extraer_kwh('Punta', texto_completo), 'llano': extraer_kwh('Llano', texto_completo), 'valle': extraer_kwh('Valle', texto_completo)}
        ...
        def limpiar_v(patron, texto):
            match = re.search(patron, texto, re.IGNORECASE)
            return float(match.group(1).replace(" ", "").replace(".", "").replace(",", ".")) if match else 0.0

        total_real = limpiar_v(r'Potencia\s+\.+\s*([\d\s.,]+)€', texto_completo) + limpiar_v(r'Energía\s+\.+\s*([\d\s.,]+)€', texto_completo)
        
        for tramo in ['Punta', 'Llano', 'Valle']:
            m = re.search(rf'{tramo}\s+[\d,.]+\s+[\d,.]+\s+[\d,.]+\s+[\w,.]+\s+([\d,.]+)', texto_completo, re.IGNORECASE)
            if m: consumos[tramo.lower()] = float(m.group(1).replace(',', '.'))
        ...
        for t in ['Punta', 'Llano', 'Valle']:
            m = re.search(rf'{t}\s*([\d,.]+)\s*kWh', texto_completo)
            if m: consumos[t.lower()] = float(m.group(1).replace(',', '.'))
        m_imp_p = re.search(r'Total\s+importe\s+potencia.*?\s*([\d,.]+)\s*€', texto_completo, re.IGNORECASE)
        m_imp_e = re.search(r'Total\s+[\d,.]+\s*kWh\s+hasta.*?\s*([\d,.]+)\s*€', texto_completo, re.IGNORECASE)
